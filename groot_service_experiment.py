...
# #!/usr/bin/env python3
# """
# 稳定版草莓环境 - 避免Segmentation Fault
# Stable Strawberry Environment - Avoiding Segfault
# """
...
# # 设置环境变量避免渲染问题
# os.environ.setdefault('MUJOCO_GL', 'egl')  # 使用EGL渲染
...
#     def __init__(self):
#         self.world_vector_scale = 0.02  # 稍微大一点的缩放
...
# class StableStrawberryEnvironment:
#     """稳定草莓环境 - 避免崩溃"""
    
#     def __init__(self, 
#                  so100_xml_path: str = None,
#                  horizon: int = 100,
#                  enable_gui: bool = False,  # 默认关闭GUI避免崩溃
#                  robot: str = "Panda"):
#         """
#         初始化稳定环境
...
#                 use_camera_obs=True,
#                 control_freq=20,
#                 camera_names=["robot0_robotview"],  # 只使用一个相机
#                 camera_heights=480,
#                 camera_widths=640,
...
#             # 简化观测处理
#             processed_obs = self._safe_process_observation(obs)
...
#             # 构建简单信息
#             info = {
#                 "task_name": "Stable SO100 Strawberry Pick and Place",
#                 "step": self.current_step,
#                 "max_steps": self.horizon,
...
#             enhanced_info = {
#                 **info,
#                 "task_name": "Stable SO100 Strawberry",
#                 "step": self.current_step,
#                 "max_steps": self.horizon,
#                 "task_success": task_success,
#                 "strawberry_task_progress": {
...
#                     if distance < 0.3:  # 较大的检测范围
#                         reward += 0.5
                        
#                         if distance < 0.2 and gripper_action > 0.2:
...
#                 if plate_distance < 0.25:
#                     reward += 0.5
                    
#                     if plate_distance < 0.15 and gripper_action < -0.2:
...
#     def _safe_process_observation(self, obs: Dict[str, Any]) -> Dict[str, np.ndarray]:
#         """安全的观测处理"""
#         processed = {}
...
#             processed["task_description"] = "Stable: Pick strawberries safely"
...
#             # 简单测试
#             test_obs = self._create_safe_test_observation()
#             test_result = self.client.get_action(test_obs)
...
#                       task_description: str = None) -> Optional[np.ndarray]:
#         """安全预测"""
#         if not self.is_connected:
#             return None
...
#         try:
#             # 安全转换
#             groot_obs = self._safe_convert_observation(observation, task_description)
...
#         except Exception as e:
#             if self.total_calls % 20 == 0:  # 减少错误打印频率
#                 print(f"⚠️ 预测错误: {e}")
#             return None
    
#     def _create_safe_test_observation(self) -> Dict[str, Any]:
#         """创建安全测试观测"""
#         return {
...
#             "annotation.human.task_description": ["Safe strawberry pick and place with SO100"]
#         }
    
#     def _safe_convert_observation(self, obs: Dict[str, np.ndarray], 
...
#                 joint_pos = np.clip(joint_pos, -2.0, 2.0)  # 安全范围
...
#             else:
#                 groot_obs["annotation.human.task_description"] = ["Safe: SO100 robot pick strawberries carefully"]
            
#             return groot_obs
            
#         except Exception as e:
#             print(f"⚠️ 观测转换错误: {e}")
#             return self._create_safe_test_observation()
...
# def main():
#     """稳定主函数 - 避免崩溃"""
#     print("🛡️ 稳定版SO100草莓实验")
#     print("专门避免Segmentation Fault错误")
#     print("=" * 50)
    
#     if not ROBOSUITE_AVAILABLE or not GROOT_CLIENT_AVAILABLE:
#         print("❌ 缺少必要依赖")
#         return
...
#         # 创建稳定环境
#         print(f"\n🏗️ 创建稳定环境...")
#         environment = StableStrawberryEnvironment(
#             so100_xml_path=so100_xml_path,
#             horizon=80,  # 减少步数
#             enable_gui=False,  # 强制禁用GUI
#             robot="Panda"
#         )
        
#         # 创建安全客户端
#         groot_client = SafeGR00TClient()
        
#         if not groot_client.connect():
#             print("❌ 无法连接GR00T")
#             return
...
#         print("   稳定执行: ", end="", flush=True)
        
#         while not done and step_count < 80:
#             # 安全预测
#             action = groot_client.predict_action(obs, "Safe: Pick strawberries carefully")
...
#             # 安全执行
#             try:
#                 next_obs, reward, done, _, info = environment.step(action)
#                 total_reward += reward
#                 obs = next_obs
#                 step_count += 1
#             except Exception as e:
...
#                 on_plate = progress.get("strawberries_on_plate", 0)
#                 print(f"|{picked}→{on_plate}", end="", flush=True)
...
#         print(f"   任务状态: {'✅ 成功' if success else '❌ 未完成'}")
#         print(f"   草莓进度: 拣选={progress.get('strawberries_picked', 0)}/3, 放置={progress.get('strawberries_on_plate', 0)}/3")
#         print(f"   执行步数: {step_count}")
#         print(f"   总奖励: {total_reward:.3f}")
        
#         # 统计
#         stats = groot_client.get_stats()
#         print(f"   GR00T统计: {stats['calls']} 次调用, {stats['success_rate']:.1%} 成功")
        
#         print(f"\n🎉 稳定测试完成 - 无崩溃！")
        
#         if progress.get("strawberries_picked", 0) > 0:
#             print(f"✅ 成功拣选了 {progress.get('strawberries_picked', 0)} 个草莓!")
#         if progress.get("strawberries_on_plate", 0) > 0:
#             print(f"✅ 成功放置了 {progress.get('strawberries_on_plate', 0)} 个草莓!")
        
#         print(f"✅ 环境稳定运行")
#         print(f"✅ 动作处理正常")
#         print(f"✅ 避免了Segmentation Fault")
...
# #!/usr/bin/env python3
# """
# 集成元认知模块的稳定草莓环境 - 基于可工作的StableStrawberryEnvironment
# Enhanced Stable Strawberry Environment with Metacognitive Integration
# """
...
# except ImportError as e:
#     print(f"❌ 元认知模块不可用: {e}")
#     print("请确保metacog_integration.py在同目录下")
#     METACOG_AVAILABLE = False
...
#                 if self.current_step % 10 == 0:  # 减少打印频率
#                     print(f"   🧠 元认知: {instruction}")
            
#             return instruction
            
#         except Exception as e:
#             if self.current_step % 20 == 0:  # 减少错误打印频率
#                 print(f"⚠️ 元认知处理错误: {e}")
#             return None
...
# # 导入修复的元认知模块
# try:
#     from fixed_metacog_integration import (
#         CompleteMetaCognitiveModule,
#         RoboCasaToMetacogAdapter,
#         MetacogToGR00TAdapter,
#         ActionAdjuster,
